enum Math<RawValue: FloatingPoint> {
    ...
    static func fractionsOfintersections(circle circle0: RawCircle<RawValue>, circle circle1: RawCircle<RawValue>, makeFraction: (RawPoint<RawValue>) -> RawValue?) -> (RawValue, RawValue)? {
        let line = RawLine(start: circle0.center, end: circle1.center)
        let delta = line.delta
        let squaredDistance = delta.squaredNorm
        let distance = delta.norm
